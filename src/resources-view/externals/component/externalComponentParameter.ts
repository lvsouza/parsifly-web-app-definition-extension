...
import { externalComponentParameter, ExternalComponentParameter, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<ExternalComponentParameter, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadExternalComponentParameter = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(externalComponentParameter).where(eq(externalComponentParameter.id, current.id));
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'external-parameter.svg' } : { path: items.length > 0 ? 'external-property-group.svg' : 'external-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'external-parameter.svg' } : { path: items.length > 0 ? 'external-property-group.svg' : 'external-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadExternalComponentParameter({
              root,
              projectId,
              current: item,
              extensionContext,
