...
import { externalActionParameter, ExternalActionParameter, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<ExternalActionParameter, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadExternalActionParameter = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(externalActionParameter).where(eq(externalActionParameter.id, current.id));
      ...
      getItems: async (context) => {
        ...
        return await Promise.all(
          items.map(async item => (
            await loadExternalActionParameter({
              root,
              projectId,
              current: item,
              extensionContext,
