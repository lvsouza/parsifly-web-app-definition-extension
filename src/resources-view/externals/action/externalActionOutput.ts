...
import { externalActionOutput, ExternalActionOutput, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<ExternalActionOutput, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadExternalActionOutput = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(externalActionOutput).where(eq(externalActionOutput.id, current.id));
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'external-output.svg' } : { path: items.length > 0 ? 'external-property-group.svg' : 'external-property.svg' },
      onItemClick: async () => {
      ...
      getContextMenuItems: async (context) => {
        return [
          ...
          new Action({
            key: isRootLevel ? 'delete-output' : 'delete-property',
            initialValue: {
              icon: { path: 'delete.svg' },
              action: handleDelete(context),
              label: isRootLevel ? 'Delete output' : 'Delete property',
              description: isRootLevel ? 'Permanently delete the output' : 'Permanently delete the property',
            },
          }),
        ];
      },
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'external-output.svg' } : { path: items.length > 0 ? 'external-property-group.svg' : 'external-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadExternalActionOutput({
              root,
              projectId,
              current: item,
              extensionContext,
