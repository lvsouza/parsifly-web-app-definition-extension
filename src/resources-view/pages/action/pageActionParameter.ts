...
export const loadPageActionParameter = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'page-parameter.svg' } : { path: items.length > 0 ? 'page-property-group.svg' : 'page-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'page-parameter.svg' } : { path: items.length > 0 ? 'page-property-group.svg' : 'page-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadPageActionParameter({
              root,
              projectId,
              current: item,
              extensionContext,
