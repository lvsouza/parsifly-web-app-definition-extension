...
import { pageParameter, PageParameter, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<PageParameter, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadPageParameter = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(pageParameter).where(eq(pageParameter.id, current.id));
