...
import { pageVariable, PageVariable, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<PageVariable, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadPageVariable = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(pageVariable).where(eq(pageVariable.id, current.id));
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'page-variable.svg' } : { path: items.length > 0 ? 'page-property-group.svg' : 'page-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'page-variable.svg' } : { path: items.length > 0 ? 'page-property-group.svg' : 'page-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadPageVariable({
              root,
              projectId,
              current: item,
              extensionContext,
