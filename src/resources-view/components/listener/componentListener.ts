...
import { componentListener, ComponentListener } from '../../../definition/schema';
...
type TProps = {
  ...
  current: Pick<ComponentListener, 'id' | 'type' | 'name'>;
};
export const loadComponentListener = async ({ extensionContext, current }: TProps): Promise<ListViewItem> => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(componentListener).where(eq(componentListener.id, current.id));
      });
