...
import { componentAction, actionParameter, ComponentAction, action, Action, property, actionOutput, actionVariable } from '../../../definition/schema';
import { createDatabaseHelper, mappableQuery } from '../../../definition/DatabaseHelper';
import { loadComponentActionParameter } from './componentActionParameter';
import { loadComponentActionVariable } from './componentActionVariable';
import { loadComponentActionOutput } from './componentActionOutput';


type TProps = {
  projectId: string;
  extensionContext: TExtensionContext;
  current: Pick<ComponentAction, 'id' | 'type' | 'actionId'> & Pick<Action, 'name' | 'description'>;
};
export const loadComponentAction = async ({ extensionContext, current, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(componentAction).where(eq(componentAction.id, current.id));
    ...
    initialValue: {
        ...
        return [
          ...
          new ActionParsifly({
            ...
            initialValue: {
              ...
              icon: { path: 'component-output.svg' },
              description: 'Add a new component output',
            },
          }),
          new ActionParsifly({
            key: 'delete-action',
      ...
      getItems: async (context) => {
        ...
        return await Promise.all([
          ...itemsParameter.map(item => loadComponentActionParameter({
            projectId,
            root: item,
            extensionContext,
            current: {
          ...
          ...itemsVariable.map(item => loadComponentActionVariable({
            projectId,
            root: item,
            extensionContext,
            current: {
          ...
          ...itemsOutput.map(item => loadComponentActionOutput({
            projectId,
            root: item,
            extensionContext,
            current: {
