...
export const loadComponentActionOutput = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'component-output.svg' } : { path: items.length > 0 ? 'component-property-group.svg' : 'component-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'component-output.svg' } : { path: items.length > 0 ? 'component-property-group.svg' : 'component-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadComponentActionOutput({
              root,
              projectId,
              current: item,
              extensionContext,
