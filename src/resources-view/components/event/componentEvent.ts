...
import { componentEvent, eventParameter, ComponentEvent, event, Event, property } from '../../../definition/schema';
import { createDatabaseHelper, mappableQuery } from '../../../definition/DatabaseHelper';
import { loadComponentEventParameter } from './componentEventParameter';


type TProps = {
  projectId: string;
  extensionContext: TExtensionContext;
  current: Pick<ComponentEvent, 'id' | 'type' | 'eventId'> & Pick<Event, 'name' | 'description'>;
};
export const loadComponentEvent = async ({ extensionContext, current, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(componentEvent).where(eq(componentEvent.id, current.id));
    ...
    initialValue: {
      ...
      getItems: async (context) => {
        ...
        return await Promise.all(
          items.map(item => loadComponentEventParameter({
            projectId,
            root: item,
            extensionContext,
            current: {
