...
import { componentVariable, ComponentVariable, NewProperty, property, Property } from '../../../definition/schema';
...
type TProps = {
  ...
  root: Pick<ComponentVariable, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadComponentVariable = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(componentVariable).where(eq(componentVariable.id, current.id));
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'component-variable.svg' } : { path: items.length > 0 ? 'component-property-group.svg' : 'component-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'component-variable.svg' } : { path: items.length > 0 ? 'component-property-group.svg' : 'component-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadComponentVariable({
              root,
              projectId,
              current: item,
              extensionContext,
