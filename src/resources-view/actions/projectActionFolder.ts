...
import { action, Folder, folder, NewFolder, projectAction } from '../../definition/schema';
...
export const loadProjectActionFolder = async ({ extensionContext, projectId, current }: TProps): Promise<ListViewItem> => {
  ...
  const loadItemsQuery = databaseHelper
    ...
    .where(eq(projectAction.parentFolderId, current.id))
    .unionAll(
      databaseHelper
        .select({
          id: folder.id,
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      getContextMenuItems: async (context) => {
        return [
          new Action({
            key: 'add-new-project-action',
            initialValue: {
              label: 'New action',
              action: handleAddItem(context),
              icon: { path: 'project-action.svg' },
              description: 'Add a new action logic',
            },
          }),
          new Action({
            key: 'add-new-folder',
