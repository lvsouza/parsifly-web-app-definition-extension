...
import { actionOutput, ActionOutput, NewProperty, property, Property } from '../../definition/schema';
...
type TProps = {
  ...
  root: Pick<ActionOutput, 'id' | 'type' | 'propertyId'> & Pick<Property, 'name' | 'description' | 'dataType'>;
};
export const loadProjectActionOutput = async ({ extensionContext, current, root, projectId }: TProps): Promise<ListViewItem> => {
  ...
  const handleDelete = (_context: TListItemMountContext) => async () => {
    try {
      await databaseHelper.transaction(async (trx) => {
        await trx.delete(actionOutput).where(eq(actionOutput.id, current.id));
  ...
  return new ListViewItem({
    ...
    initialValue: {
      ...
      icon: isRootLevel ? { path: 'project-action-output.svg' } : { path: items.length > 0 ? 'project-action-property-group.svg' : 'project-action-property.svg' },
      onItemClick: async () => {
      ...
      getItems: async (context) => {
        ...
        await context.set('icon', isRootLevel ? { path: 'project-action-output.svg' } : { path: items.length > 0 ? 'project-action-property-group.svg' : 'project-action-property.svg' });
        return await Promise.all(
          items.map(async item => (
            await loadProjectActionOutput({
              root,
              projectId,
              current: item,
              extensionContext,
