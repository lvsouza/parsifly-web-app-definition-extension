...
import { externalComponentParameter, property, TWebAppDataType } from '../../definition/schema';
import { createDatabaseHelper, mappableQuery } from '../../definition/DatabaseHelper';


export const createExternalComponentParameterFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-externalComponentParameter-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'externalComponentParameter') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, externalComponentParameterId: sql<string | undefined>`${externalComponentParameter.id}`.as('externalComponentParameterId') })
        .from(property)
        .leftJoin(externalComponentParameter, eq(externalComponentParameter.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'External component parameter',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
