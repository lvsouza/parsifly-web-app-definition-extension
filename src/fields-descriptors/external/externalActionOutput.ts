...
import { externalActionOutput, property, TWebAppDataType } from '../../definition/schema';
import { createDatabaseHelper } from '../../definition/DatabaseHelper';


export const createExternalActionOutputFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-externalActionOutput-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'externalActionOutput') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, externalActionOutputId: sql<string | undefined>`${externalActionOutput.id}`.as('externalActionOutputId') })
        .from(property)
        .leftJoin(externalActionOutput, eq(externalActionOutput.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'External action output',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change output name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change output description',
            getValue: async () => {
              const [item] = await databaseHelper
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            label: 'Data type',
            type: 'autocomplete',
            description: 'Change output data type',
