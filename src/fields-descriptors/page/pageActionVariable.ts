...
export const createPageActionVariableFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-pageActionVariable-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'pageActionVariable') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, pageActionVariableId: sql<string | undefined>`${actionVariable.id}`.as('pageActionVariableId') })
        .from(property)
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Page action variable',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
