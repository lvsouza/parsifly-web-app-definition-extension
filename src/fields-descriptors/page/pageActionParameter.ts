...
export const createPageActionParameterFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-pageActionParameter-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'pageActionParameter') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, pageActionParameterId: sql<string | undefined>`${actionParameter.id}`.as('pageActionParameterId') })
        .from(property)
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Page action parameter',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
