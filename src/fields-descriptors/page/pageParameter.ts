...
import { pageParameter, property, TWebAppDataType } from '../../definition/schema';
import { createDatabaseHelper } from '../../definition/DatabaseHelper';


export const createPageParameterFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-pageParameter-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'pageParameter') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, pageParameterId: sql<string | undefined>`${pageParameter.id}`.as('pageParameterId') })
        .from(property)
        .leftJoin(pageParameter, eq(pageParameter.propertyId, property.id))
      ...
      return [
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change page parameter name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change page parameter description',
            getValue: async () => {
              const [item] = await databaseHelper
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'autocomplete',
            label: 'Data type',
            description: 'Change page parameter data type',
