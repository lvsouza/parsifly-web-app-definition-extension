...
export const createPageActionOutputFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-pageActionOutput-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'pageActionOutput') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, pageActionOutputId: sql<string | undefined>`${actionOutput.id}`.as('pageActionOutputId') })
        .from(property)
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Page action output',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
