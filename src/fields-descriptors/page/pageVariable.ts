...
import { pageVariable, property, TWebAppDataType } from '../../definition/schema';


export const createPageVariableFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-pageVariable-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'pageVariable') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, pageVariableId: sql<string | undefined>`${pageVariable.id}`.as('pageVariableId') })
        .from(property)
        .leftJoin(pageVariable, eq(pageVariable.propertyId, property.id))
      ...
      return [
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change page variable name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change page variable description',
            getValue: async () => {
              const [item] = await databaseHelper
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'autocomplete',
            label: 'Data type',
            description: 'Change page variable data type',
