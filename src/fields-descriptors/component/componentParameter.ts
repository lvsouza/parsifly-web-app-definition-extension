...
import { componentParameter, property, TWebAppDataType } from '../../definition/schema';
import { createDatabaseHelper, mappableQuery } from '../../definition/DatabaseHelper';


export const createComponentParameterFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentParameter-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentParameter') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, componentParameterId: sql<string | undefined>`${componentParameter.id}`.as('componentParameterId') })
        .from(property)
        .leftJoin(componentParameter, eq(componentParameter.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component parameter',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change component parameter name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change component parameter description',
            getValue: async () => {
              const [item] = await databaseHelper
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'autocomplete',
            label: 'Data type',
            description: 'Change component parameter data type',
