...
import { actionOutput, property, TWebAppDataType } from '../../definition/schema';


export const createComponentActionOutputFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentActionOutput-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentActionOutput') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, componentActionOutputId: sql<string | undefined>`${actionOutput.id}`.as('componentActionOutputId') })
        .from(property)
        .leftJoin(actionOutput, eq(actionOutput.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component action output',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
