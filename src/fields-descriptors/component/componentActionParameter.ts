...
export const createComponentActionParameterFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentActionParameter-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentActionParameter') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, componentActionParameterId: sql<string | undefined>`${actionParameter.id}`.as('componentActionParameterId') })
        .from(property)
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component action parameter',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
