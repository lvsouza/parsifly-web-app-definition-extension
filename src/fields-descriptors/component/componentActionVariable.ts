...
export const createComponentActionVariableFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentActionVariable-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentActionVariable') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, componentActionVariableId: sql<string | undefined>`${actionVariable.id}`.as('componentActionVariableId') })
        .from(property)
        .leftJoin(actionVariable, eq(actionVariable.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component action variable',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
