...
export const createComponentActionFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentAction-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentAction') return [];
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component action',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change component action name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change component action description',
            getValue: async () => {
              const [item] = await databaseHelper
