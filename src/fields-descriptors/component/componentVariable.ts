...
import { componentVariable, property, TWebAppDataType } from '../../definition/schema';
import { createDatabaseHelper, mappableQuery } from '../../definition/DatabaseHelper';


export const createComponentVariableFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-componentVariable-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'componentVariable') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, componentVariableId: sql<string | undefined>`${componentVariable.id}`.as('componentVariableId') })
        .from(property)
        .leftJoin(componentVariable, eq(componentVariable.propertyId, property.id))
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Component variable',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
          initialValue: {
            ...
            type: 'text',
            label: 'Name',
            description: 'Change component variable name',
            getValue: async () => {
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            name: 'description',
            label: 'Description',
            description: 'Change component variable description',
            getValue: async () => {
              const [item] = await databaseHelper
        ...
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'autocomplete',
            label: 'Data type',
            description: 'Change component variable data type',
