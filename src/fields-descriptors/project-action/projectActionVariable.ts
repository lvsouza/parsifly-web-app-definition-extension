...
export const createProjectActionVariableFieldsDescriptor = (extensionContext: TExtensionContext) => {
  ...
  return new FieldsDescriptor({
    key: 'web-app-projectActionVariable-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'projectActionVariable') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, actionVariableId: sql<string | undefined>`${actionVariable.id}`.as('actionVariableId') })
        .from(property)
      ...
      return [
        new FieldViewItem({
          ...
          initialValue: {
            ...
            type: 'view',
            label: 'Type',
            getValue: async () => 'Action variable',
          },
        }),
        new FieldViewItem({
          key: `name:${result.id}`,
