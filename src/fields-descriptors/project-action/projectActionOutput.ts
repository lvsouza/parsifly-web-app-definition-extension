...
export const createProjectActionOutputFieldsDescriptor = (extensionContext: TExtensionContext) => {
  const databaseHelper = createDatabaseHelper(extensionContext);

  return new FieldsDescriptor({
    key: 'web-app-projectActionOutput-fields-descriptor',
    onGetFields: async (intent) => {

      const [target] = intent.targets
      if (target.kind !== 'projectActionOutput') return [];

      const [result] = await databaseHelper
        .select({ id: property.id, actionOutputId: sql<string | undefined>`${actionOutput.id}`.as('actionOutputId') })
        .from(property)
